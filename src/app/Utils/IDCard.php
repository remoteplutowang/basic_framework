...
class IDCard
{
  ...
  public function getGender($id_number)
  {
    // 获取身份证倒数第二位数字
    $number = substr($id_number, strlen($id_number) - 2, 1);
    if ($number % 2 == 0) { // 偶数女
      $ret =  2;
    } else { // 奇数男
      $ret =  1;
    ...
    public function getBirthday($id_number)
    {
        $retValue = "";
        $pattern = '/^(\d{6})(\d{8})(.*)$/';
        if (preg_match($pattern, $id_number, $matches)) {
            $birthday = $matches[2];
            $retValue =date("Y-m-d",strtotime($birthday));
        }
        return $retValue;
    }
    ...
    public function fakeID()
    {
        ...
        $valueArray = explode(".",$microtime);
        $fakeID = $valueArray[0].$valueArray[1] . rand(100,999)."F";
        return $fakeID;
    }
}
